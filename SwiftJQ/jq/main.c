...
//      jv_dump(result, dumpopts);
      jv dump_str = jv_dump_string(result, dumpopts);
      if (jv_get_kind(dump_str) == JV_KIND_STRING) {
        fprintf(stdout, "%s", jv_string_value(dump_str));
      }
      jv_free(dump_str);
    }
...
#if defined(USE_BUFFERS)
      jq_util_input_add_buffer_from_file(input_state, argv[i]);
#else
