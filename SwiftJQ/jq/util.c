...
jv jq_realpath(jv path) {
  ...
  char *buf = NULL;
#ifdef _PC_PATH_MAX
  // XXXMEH - fix implicit conversion warning
  path_max = (int) pathconf(jv_string_value(path),_PC_PATH_MAX);
#else
  path_max = PATH_MAX;
#endif
  if (path_max > 0) {
...
struct jq_util_input_state {
  ...
  int nbuffers;
  int curr_buffer;
  unsigned char* current_input_buffer;
  int current_input_buffer_offset;
...
jq_util_input_state *jq_util_input_init(jq_util_msg_cb err_cb, void *err_cb_data) {
  ...
  new_state->buffers = NULL;
  new_state->buffer_sizes = NULL;
  new_state->nbuffers = 0;
  new_state->curr_buffer = 0;
  new_state->current_input_buffer = NULL;
  new_state->current_input_buffer_offset = 0;
  new_state->current_input_buffer_size = 0;
...
void jq_util_input_free(jq_util_input_state **state) {
  ...
  *state = NULL;
  ...
  for (int i = 0; i < old_state->nbuffers; i++)
    free(old_state->buffers[i]);
  free(old_state->buffers);
  free(old_state->buffer_sizes);
...
void jq_util_input_add_buffer_from_file(jq_util_input_state *state, const char *fname) {
    // Open the file
    ...
    if (fp != NULL) {
        ...
        if (file_buffer != NULL) {
            ...
            fread(file_buffer, sizeof(unsigned char), file_size, fp);
            // Now call jq_util_input_add_buffer to add the buffer
            jq_util_input_add_buffer(state, file_buffer, file_size);
        }
        free(file_buffer);
    }
    fclose(fp);
...
void jq_util_input_add_buffer(jq_util_input_state *state, unsigned char* buffer, int buffer_size) {
    unsigned char* buffer_copy = jv_mem_alloc(buffer_size);
    memcpy(buffer_copy, buffer, buffer_size);
    state->buffers = jv_mem_realloc(state->buffers, (state->nbuffers + 1) * sizeof(unsigned char*));
    state->buffers[state->nbuffers] = buffer_copy;
    state->buffer_sizes = jv_mem_realloc(state->buffer_sizes, (state->nbuffers + 1) * sizeof(int));
    state->buffer_sizes[state->nbuffers] = buffer_size;
    state->nbuffers++;
}
...
static unsigned char* next_buffer(jq_util_input_state *state, int *next_buffer_size, int *next_buffer_index) {
  if (state->curr_buffer < state->nbuffers) {
    if (next_buffer_size != NULL) {
        *next_buffer_size = state->buffer_sizes[state->curr_buffer];
    }
    if (next_buffer_index != NULL) {
        *next_buffer_index = state->curr_buffer;
    }
    return state->buffers[state->curr_buffer++];
  }
  return NULL;
}

static int has_files(jq_util_input_state *state) {
    return (state->nbuffers > 0 ? 0 : 1);
}

static int jq_util_input_read_more_from_files(jq_util_input_state *state) {
...
static int jq_util_input_end_of_buffer(jq_util_input_state *state) {
    int at_eof = 0;

    if (state->current_input_buffer != NULL &&
        state->current_input_buffer_size > 0 &&
        state->current_input_buffer_offset >= state->current_input_buffer_size) {
        at_eof = 1;
    }
...
static int jq_util_input_read_more_from_buffers(jq_util_input_state *state) {
    if (!state->current_input_buffer || jq_util_input_end_of_buffer(state)) {
        if (state->current_input_buffer != NULL) {
            state->current_input_buffer = NULL;
            state->current_line = 0 ;
            jv_free(state->current_filename);
            state->current_filename = jv_invalid();
        }
        int next_buffer_size = 0;
        int buffer_index = 0;
        unsigned char *next_buf = next_buffer(state, &next_buffer_size, &buffer_index);
        if (next_buf != NULL) {
            state->current_input_buffer = next_buf;
            state->current_input_buffer_offset = 0;
            state->current_input_buffer_size = next_buffer_size;
            state->current_line = 0;
            char file_name[32];
            sprintf(&file_name[0], "buffer[%d]", buffer_index);
            state->current_filename = jv_string(file_name);
        }
    }
    ...
    state->buf_valid_len = 0;
    if (state->current_input_buffer != NULL) {
        // NUL out the entire line buffer
        memset(state->buf, 0, sizeof(state->buf));
        // Compute how many bytes are left in our current buffer
        unsigned char *current_buffer_cursor = state->current_input_buffer + state->current_input_buffer_offset;
        int current_buffer_bytes_left = state->current_input_buffer_size - state->current_input_buffer_offset;
        // Search for a newline in the remaining buffer
        unsigned char* newline = (unsigned char *)memchr(current_buffer_cursor, '\n', current_buffer_bytes_left);
        // Determine the number of bytes to copy (include the newline)
        int num_bytes_to_copy = (newline != NULL ? (int) (newline - current_buffer_cursor + 1) : current_buffer_bytes_left);
        // Compute the line buf size, but leave room for last NUL character
        int line_buf_size = sizeof(state->buf) - 1;
        num_bytes_to_copy = (num_bytes_to_copy <= line_buf_size ? num_bytes_to_copy : line_buf_size);

        // Copy from the current buffer into the line buffer
        memcpy(state->buf, current_buffer_cursor, num_bytes_to_copy);

        // Advance the current buffer offset
        state->current_input_buffer_offset = state->current_input_buffer_offset + num_bytes_to_copy;

        // Now search for new line in line buf
        const char *p = memchr(state->buf, '\n', sizeof(state->buf));
        if (p != NULL) {
            state->current_line++;
        }
        ...
        if (p == NULL && state->parser != NULL) {
             ...
             */
            state->buf_valid_len = strlen(state->buf);
        } else if (p == NULL && jq_util_input_end_of_buffer(state)) {
            size_t i;
    ...
    return state->curr_buffer == state->nbuffers &&
           (!state->current_input_buffer || jq_util_input_end_of_buffer(state));
}

static int jq_util_input_read_more(jq_util_input_state *state) {
    if (has_files(state)) {
        return jq_util_input_read_more_from_files(state);
    } else {
        return jq_util_input_read_more_from_buffers(state);
    }
}
  ...
  do {
    if (state->parser == NULL) {
      ...
      if (jv_is_valid(state->slurped)) {
        // Slurped raw input
        // XXXMEH - fix implicit conversion warning
        state->slurped = jv_string_concat(state->slurped, jv_string_sized(state->buf, (int) state->buf_valid_len));
      } else {
        ...
        if (state->buf[state->buf_valid_len-1] == '\n') {
          ...
          // XXXMEH - fix implicit conversion warning
          return jv_string_concat(value, jv_string_sized(state->buf, (int) state->buf_valid_len-1));
        }
        // XXXMEH - fix implicit conversion warning
        value = jv_string_concat(value, jv_string_sized(state->buf, (int) state->buf_valid_len));
        state->buf[0] = '\0';
        state->buf_valid_len = 0;
      }
    } else {
